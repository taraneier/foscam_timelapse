...
while [ $(( $end - $start )) -lt $(( 2 * 6 )) ]; do
    equals=$(expr $end - $start)
    echo "${end} - ${start} = ${equals}"
...
# ffmpeg -loglevel quiet -y -rtsp_transport tcp -i rtsp://nadmin:74xM3LafP3Lw@192.168.0.241:88/videoMain -vsync 2 -frames:v 1 -strftime 1 %Y-%m-%d_%H-%M-%S.jpg"
